...
export default {
  ...
  mounted() {
    this.showproducts(this.selecteditem);
  },
  methods: {
    addclass(index) {
      console.log(index);
    ...
    async showproducts(item) {
      ...
      try {
        const res = await productsApi.getProducts(item._id);
        this.productData = res.data.products;
        const ress = await productsApi.getCategories(item._id);
    ...
    async delProductImg() {
      try {
        ...
        console.log(this.ProductImg.id);
        console.log(imgSrc);
        const res = await productsApi.deleteProductImg(this.ProductImg.id, { imgSrc });
        console.log(res);
        this.loading = false;
      } catch (err) {
        this.loading = false;
