  ...
  <div>
    <v-card width="600" class="pa-6 mx-auto mt-7">
        ...
        <v-card class="pa-5">
          <v-card-actions class="d-flex justify-content-center">
            <v-btn
              ...
              :loading="loading"
              >{{ cuurentuser.img ? "تأكيد تغيير الصورة" : "تأكيد وضع صورة جديدة" }}</v-btn
            >
      ...
      <div class="d-flex justify-content-center mb-3">
        <v-btn class="success font-weight-bold" @click="openChangeInfo(cuurentuser)"
    ...
    <v-dialog width="600" v-model="dilogInfo">
      <v-card class="pa-5">
        ...
        <v-row justify="center" aling="center">
          <v-col cols="12" md="8" class="d-flex justify-center pb-4">
            <v-btn class="info font-weight-bold" @click="saveChange()" :loading="loading"
...
export default {
  ...
  methods: {
    ...
    async saveChangePass(newpass) {
      ...
      try {
        await profileApi.changePassword(newpass);
      } catch (error) {
        console.log(error);
      }
      this.loading = false;
