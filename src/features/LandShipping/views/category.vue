          ...
          <v-dialog v-model="openDeleteCategory" width="500">
            <v-card class="pa-5">
              ...
              <v-card-actions class="d-flex justify-content-center">
                <v-btn class="success font-weight-bold" :loading="loading" @click="sureDelCategory"
                  >تأكيد</v-btn
                >
...
export default {
  ...
  mounted() {
    this.showcategory(this.selecteditem);
  },
  methods: {
    async addcategory() {
    ...
    async showcategory(item) {
      ...
      try {
        ...
        const res = await categoriesApi.getCategory(item._id);
